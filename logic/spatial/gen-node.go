...
func (n *IntNode) Collect(group int, including bool, coll []int) []int {
	if including {
		for _, g := range n.Groups {
			if g.ID == group {
				coll = append(coll, n.Ints[g.Idx:g.Idx+g.Len]...)
				break
			}
		}
	} else {
...
func (n *IntNode) Insert(item int, group int) {
	...
	if g == nil {
		n.Groups = append(n.Groups, IntGroup{
			ID:  group,
			Idx: len(n.Ints),
		})
	...
	n.Ints = append(n.Ints, item)
	copy(n.Ints[g.Idx+1:], n.Ints[g.Idx:])
	n.Ints[g.Idx] = item
}
...
func (n *IntNode) Remove(item int, group int) bool {
	...
	for i := g.Idx; ; i++ {
		...
		if n.Ints[i] == item {
			n.Ints = append(n.Ints[:i], n.Ints[i+1:]...)
			break
		}
	}
...
// IntNode keeps over all count of ids in sets for quick lookup
...
// Insert ...
func ( n * IntNode ) Insert ( id int , group int ) {
	n.Count++
	for i := range n.Sets {
		s := &n.Sets[i]
		if s.Group == group {
...
n . Sets = append ( n . Sets , IntSet { group , [ ] int { id } } ) ;
...
func ( n * IntNode ) Remove ( id int , group int ) bool {
	n.Count--
	ll := len(n.Sets)
...
func ( n * IntNode ) Collect ( group int , include bool , coll [ ] int ) [ ] int {
	if include {
...
func ( n * IntNode ) CollectAll ( coll * [ ] int ) {
	for _, s := range n.Sets {
		*coll = append(*coll, s.IDs...)
	}
}

// IntSet is an id set that also has a group important for node
type IntSet struct {
	Group int
IDs [ ] int ;
}*/
